...
function sanitizeSvg(html: string): string {
  ...
  const events = ['onclick', 'onerror', 'onload', 'onmouseover', 'onfocus', 'onblur']
...
type WeatherType = 'sunny' | 'cloudy' | 'lightRain' | 'heavyRain' | 'stormy' | 'snowy' | 'blizzard' | 'windy' | 'typhoon' | 'foggy'

interface WeatherData {
  city: string; temperature: number; condition: string; type: WeatherType
  humidity: number; windSpeed: number; feelsLike: number
}

interface Props { data: WeatherData; onDismiss: () => void }
...
const S = {
  inner: { position: 'relative' as const, zIndex: 2, padding: '16px 14px 14px' },
  cityRow: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 },
  city: { fontSize: 12, fontWeight: 600, color: 'rgba(255,255,255,0.7)', letterSpacing: 0.4 },
  time: { fontSize: 11, fontWeight: 400, color: 'rgba(255,255,255,0.4)' },
  main: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 },
  iconWrap: { width: 44, height: 44, display: 'flex', alignItems: 'center', justifyContent: 'center' },
  tempBlock: { textAlign: 'right' as const },
  temp: { fontSize: 42, fontWeight: 200, lineHeight: 1, color: 'rgba(255,255,255,0.88)', letterSpacing: -2 },
  unit: { fontSize: 15, fontWeight: 200, color: 'rgba(255,255,255,0.45)', verticalAlign: 'super' },
  desc: { fontSize: 11, fontWeight: 500, color: 'rgba(255,255,255,0.6)', marginBottom: 2 },
  feels: { fontSize: 10, color: 'rgba(255,255,255,0.35)', marginBottom: 10 },
  divider: { height: 1, background: 'linear-gradient(to right, transparent, rgba(255,255,255,0.08), transparent)', marginBottom: 10 },
  grid: { display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6 },
  cell: { display: 'flex', alignItems: 'center', gap: 6, padding: '7px 8px', borderRadius: 10, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.05)' },
  cellLabel: { fontSize: 9, color: 'rgba(255,255,255,0.35)' },
  cellValue: { fontSize: 11, color: 'rgba(255,255,255,0.65)', fontWeight: 500 },
  animLayer: { position: 'absolute' as const, inset: 0, overflow: 'hidden', pointerEvents: 'none' as const, borderRadius: 18 },
  topLine: { position: 'absolute' as const, top: 0, left: 14, right: 14, height: 1, zIndex: 10, background: 'linear-gradient(90deg, transparent, rgba(255,255,255,0.12), transparent)' },
}
...
function AnimLayer({ type }: { type: WeatherType }) {
  ...
  if (particles.current.length === 0) {
    ...
    const D = (cls:string, st:any) => <div className={cls} style={st} />
    if (type === 'lightRain') {
      for (let i=0;i<10;i++) p.push(D('',{position:'absolute',top:-6,width:1,height:12,left:`${6+i*10}%`,animationDelay:`${i*0.12}s`,background:'linear-gradient(to bottom, transparent, rgba(200,210,230,0.4))',borderRadius:1}))
    } else if (type === 'heavyRain') {
      for (let i=0;i<22;i++) p.push(D('',{position:'absolute',top:-6,width:1.5,height:16,left:`${Math.random()*100}%`,animationDelay:`${Math.random()*0.5}s`,background:'linear-gradient(to bottom, transparent, rgba(180,195,220,0.5))',borderRadius:1}))
    } else if (type === 'stormy') {
      for (let i=0;i<22;i++) p.push(D('',{position:'absolute',top:-6,width:1.5,height:16,left:`${Math.random()*100}%`,animationDelay:`${Math.random()*0.5}s`,background:'linear-gradient(to bottom, transparent, rgba(180,195,220,0.5))',borderRadius:1}))
    } else if (type === 'snowy') {
      for (let i=0;i<18;i++) p.push(D('',{position:'absolute',top:-6,width:3,height:3,left:`${Math.random()*100}%`,animationDelay:`${Math.random()*5}s`,animationDuration:`${4+Math.random()*2}s`,background:'rgba(255,255,255,0.5)',borderRadius:'50%',boxShadow:'0 0 4px rgba(255,255,255,0.15)'}))
    } else if (type === 'blizzard') {
      for (let i=0;i<28;i++) p.push(D('',{position:'absolute',top:-6,width:4,height:4,left:`${Math.random()*100}%`,animationDelay:`${Math.random()*3}s`,animationDuration:`${2+Math.random()*1.5}s`,background:'rgba(255,255,255,0.65)',borderRadius:'50%',boxShadow:'0 0 6px rgba(255,255,255,0.2)'}))
    } else if (type === 'windy') {
      for (let i=0;i<6;i++) p.push(D('',{position:'absolute',left:-40,height:1,top:`${14+i*15}%`,width:`${50+Math.random()*30}px`,animationDelay:`${i*0.3}s`,animationDuration:`${1.4+Math.random()*0.5}s`,background:'linear-gradient(to right, transparent, rgba(255,255,255,0.18), transparent)',borderRadius:1}))
    } else if (type === 'typhoon') {
      for (let i=0;i<25;i++) p.push(D('',{position:'absolute',top:-6,width:1.5,height:14,left:`${Math.random()*100}%`,animationDelay:`${Math.random()*0.4}s`,animationDuration:`${0.4+Math.random()*0.2}s`,background:'linear-gradient(to bottom, transparent, rgba(180,200,230,0.45))',borderRadius:1}))
      for (let i=0;i<4;i++) p.push(D('',{position:'absolute',left:-40,height:1,top:`${14+i*18}%`,width:`${60+Math.random()*40}px`,animationDelay:`${i*0.2}s`,animationDuration:`${1+Math.random()*0.4}s`,background:'linear-gradient(to right, transparent, rgba(255,255,255,0.18), transparent)',borderRadius:1}))
    } else if (type === 'foggy') {
      for (let i=0;i<4;i++) p.push(D('',{position:'absolute',left:0,right:0,height:28,top:`${i*22}%`,animationDelay:`${i*0.9}s`,background:'linear-gradient(to right, transparent, rgba(200,200,210,0.05), transparent)',borderRadius:14}))
    }
    particles.current = p
  }
  return <div style={S.animLayer}>{particles.current}{type==='stormy'&&<StormFlash/>}</div>
}

function StormFlash() {
  const [v,setV]=useState(false)
  useEffect(()=>{const t=setInterval(()=>{setV(true);setTimeout(()=>setV(false),60)},2200+Math.random()*1500);return ()=>clearInterval(t)},[])
  return <>{v&&<div style={{position:'absolute',inset:0,zIndex:5,background:'rgba(255,255,255,0.06)',pointerEvents:'none'}}/>}<div style={{position:'absolute',top:10,left:'45%',width:2,height:36,background:'linear-gradient(to bottom, rgba(255,255,255,0.4), transparent)',transform:'translateX(-50%)',opacity:0}}/></>
}

export default function WeatherCard({ data, onDismiss }: Props) {
  const [time, setTime] = useState('')
  const [hovered, setHovered] = useState(false)
  useEffect(() => {
    const u=()=>{const n=new Date();setTime(`${String(n.getHours()).padStart(2,'0')}:${String(n.getMinutes()).padStart(2,'0')}`)}
  ...
  return (
    ...
    >
      ...
      <div style={S.inner}>
        ...
        <div style={S.main}>
          <div style={S.iconWrap} dangerouslySetInnerHTML={{__html: sanitizeSvg(ICONS[data.type] || '')}}/>
          <div style={S.tempBlock}>
            <span style={S.temp}>{data.temperature}</span><span style={S.unit}>°</span>
          </div>
        </div>
        ...
        <div style={S.grid}>
          <div style={S.cell}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.45)" strokeWidth="1.5"><path d="M12 2.69l5.66 5.66a8 8 0 1 1-11.31 0z"/></svg>
            <div><div style={S.cellLabel}>湿度</div><div style={S.cellValue}>{data.humidity}%</div></div>
          </div>
          <div style={S.cell}>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.45)" strokeWidth="1.5"><path d="M9.59 4.59A2 2 0 1 1 11 8H2m10.59 8.41A2 2 0 1 0 14 16H2m15.73-8.27A2.5 2.5 0 1 1 19.5 12H2"/></svg>
            <div><div style={S.cellLabel}>风速</div><div style={S.cellValue}>{data.windSpeed}km/h</div></div>
          </div>
        </div>
      </div>
    </div>
