...
IMAGE_HOST = os.getenv("IMAGE_HOST", "http://10.10.100.10:1666/cache")
...
async def _upload_image(filepath: str) -> str:
    ...
    if not IMAGE_HOSTS:
        raise GenerateError("没有可用的图床配置，请检查 IMAGE_HOSTS 环境变量")
...
class ImageSession:
    ...
    async def generate(
        ...
        *,
        ...
        Args:
            prompt: 图片描述（中文原文直传）
            aspect_ratio: 宽高比 (1:1, 4:3, 16:9, 9:16, 3:4)
        ...
        Raises:
            GenerateError: 出图失败
        """
        logger.info(f"生成图片: prompt={prompt[:80]} ratio={aspect_ratio} size={image_size} refs={bool(ref_urls)}")

        mcp = get_mcp()
        tool_name = "generate_image_model1"
        args = {"prompt": prompt, "aspectRatio": aspect_ratio, "imageSize": image_size}
    ...
    async def modify(
        ...
        *,
        ...
        if os.path.isabs(self._last_url):
            fpath = self._last_url
        else:
        ...
        # 继承上一张图的参数
        if aspect_ratio is None and self._history:
            aspect_ratio = self._history[-1].get("aspect_ratio", "16:9")
...
async def _cli():
    ...
    if len(sys.argv) < 2:
        print("用法: python gen.py '图片描述' [--ratio 16:9] [--size 2K] [--ref URL]")
        print("示例: python gen.py '一只猫' --ratio 1:1 --size 2K")
        sys.exit(1)
    ...
    try:
        result = await generate(prompt, aspect_ratio=ratio, image_size=size, ref_urls=refs or None)
        print(result)
